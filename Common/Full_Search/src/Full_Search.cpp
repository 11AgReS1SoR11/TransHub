...
template<typename T>
bool is_all_zero(const std::vector<T>& A)
{
    for (const auto& i : A)
    {
        if (i != 0) return false;
 ...
 */
template<typename T>
void Full_search_rec(std::vector<T>& storagesItems, std::vector<T>& clientsNeeds, std::vector<T>& couriersCapacity, const Mtx::Matrix<T>& roadsCouriersToStorages,
...
template Rout_and_length<int> Full_search(std::vector<int>& storagesItems, std::vector<int>& clientsNeeds, std::vector<int>& couriersCapacity,
                                const Mtx::Matrix<int>& roadsCouriersToStorages, const Mtx::Matrix<int>& roadsStoragesToClients);
template Rout_and_length<double> Full_search(std::vector<double>& storagesItems, std::vector<double>& clientsNeeds, std::vector<double>& couriersCapacity,
                                const Mtx::Matrix<double>& roadsCouriersToStorages, const Mtx::Matrix<double>& roadsStoragesToClients);
template void Full_search_rec(std::vector<int>& storagesItems, std::vector<int>& clientsNeeds, std::vector<int>& couriersCapacity, const Mtx::Matrix<int>& roadsCouriersToStorages,
                        const Mtx::Matrix<int>& roadsStoragesToClients, int result, Mtx::Matrix<int>& result_routes, int& g_min, Mtx::Matrix<int>& g_routes);
template void Full_search_rec(std::vector<double>& storagesItems, std::vector<double>& clientsNeeds, std::vector<double>& couriersCapacity, const Mtx::Matrix<double>& roadsCouriersToStorages,
...
template bool is_all_zero(const std::vector<int>& A);
template bool is_all_zero(const std::vector<double>& A);
