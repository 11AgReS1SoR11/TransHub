...
bool DailyRollingFileAppender::checkEntryConditions() const
{
    if (mActiveDatePattern.isEmpty())
    {
        LogError e = LOG4QT_QCLASS_ERROR("Use of appender '%1' without having a valid date pattern set",
...
void DailyRollingFileAppender::computeFrequency()
{
    ...
    {
        LogError e = LOG4QT_QCLASS_ERROR("The pattern '%1' does not specify a frequency for appender '%2'",
