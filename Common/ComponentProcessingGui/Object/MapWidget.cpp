...
void MapWidget::slotUpdated (const QString &/*guid*/, int /*type*/)
{
    if (!_model) {
...
//    }
    ...
    QTimer::singleShot (0, this, &MapWidget::updateSpan);
}

void MapWidget::slotUpdateFailed (const QString &/*msg*/, int /*type*/)
{
    //showMessage (tr ("Data access error."), MessageWidget::Error);
}

void MapWidget::slotInit ()
{
...
//    _model->setHorizontalHeaderItem (Columns::Status, new QStandardItem (tr ("Status")));                            //-- Статус
//    _model->setHorizontalHeaderItem (Columns::MapNumber, new QStandardItem (tr ("Map\nnumber")));            //-- Номер команды
//    _model->setHorizontalHeaderItem (Columns::ReceiverCallsign, new QStandardItem (tr ("Receiver\ncallsign")));      //-- Узел связи корреспондента
//    _model->setHorizontalHeaderItem (Columns::Production, new QStandardItem (tr ("Production")));                    //-- Технология (АПД Оникс, Созвездие, ОНИИП)
//    _model->setHorizontalHeaderItem (Columns::CommunicationType, new QStandardItem (tr ("Communication\ntype")));    //-- Вид связи
//    _model->setHorizontalHeaderItem (Columns::InterfaceType, new QStandardItem (tr ("Interface\ntype")));            //-- Тип стыка
//    _model->setHorizontalHeaderItem (Columns::Mode, new QStandardItem (tr ("Mode")));                                //-- Режим работы
//    _model->setHorizontalHeaderItem (Columns::StartDate, new QStandardItem (tr ("Start date")));                     //-- Дата начала
//    _model->setHorizontalHeaderItem (Columns::FreqsRx, new QStandardItem (tr ("Freqs RX")));                         //-- Частоты
//    _model->setHorizontalHeaderItem (Columns::FreqsTx, new QStandardItem (tr ("Freqs TX")));                         //-- Частоты
//    _model->setHorizontalHeaderItem (Columns::ReceiveDate, new QStandardItem (tr ("Receive date")));                 //-- Время поступления
//    _model->setHorizontalHeaderItem (Columns::Operator, new QStandardItem (tr ("Operator")));                        //-- Оператор
...
//             this, &MapWidget::slotRequestContextMenu);
...
//    }
    ...
    connect (ui->tableView_->selectionModel (), &QItemSelectionModel::selectionChanged,
             this, &MapWidget::slotSelectionChanged);
...
void MapWidget::slotRequestContextMenu (const QPoint &/*pos*/)
{
}

void MapWidget::updateSpan ()
{
...
void MapWidget::updateToolbar ()
{
    qCritical () << "[MapWidget][updateToolbar] UNDER CONSTRUCTION Do it!";
}

void MapWidget::slotShowClicked ()
{
}

void MapWidget::slotSelectionChanged (const QItemSelection &se, const QItemSelection &/*de*/)
{
...
void MapWidget::createToolBar ()
{
    ...
    _toolBar->addAction (QIcon (":/icons/show_32x32.png"), tr ("Show"), this, &MapWidget::slotShowClicked);
